...
statement = connection.createStatement(); 
		 ...
		 if(request.getParameter("textNAME") != null){
				 command = "UPDATE USER SET NAME = '" + request.getParameter("textNAME") + "', PASS = '" + request.getParameter("textPASS") + "', PROFILE = '" + request.getParameter("textPROFILE") + "', STATUS = " + request.getParameter("textSTATUS") + " WHERE userID = " + request.getParameter("userID");
           try {
...
connection = DriverManager.getConnection(connectionURL, "admin", "admin"); 
statement = connection.createStatement(); 
rsa = statement.executeQuery("SELECT * FROM user WHERE userID = "  + request.getParameter("userID")); 
...
<% } %></div>
    ...
    <div id="content"><!-- InstanceBeginEditable name="work" --><a href="admin_index.jsp">Datenbank</a> | <a href="admin_user.jsp">User verwalten</a> &gt; <strong>User mutieren</strong><br />
      <br />
      <font size="4"><strong>User verwalten<br />
        ...
        <table width="50%" border="0">
          <tr>
			...
			<% while (rsa.next()) {  %>
<form id="form1" name="form1" method="post" action="admin_mut_user.jsp?userID=<%= rsa.getString("userID") %>">
          <tr>
            <td width="15%" align="right">ID: </td>
            <td width="15%"><%= rsa.getString("userID") %></td>
          </tr>
          <tr>
            <td align="right">Name: </td>
            <td><input name="textNAME" type="text" id="textNAME" value="<%= rsa.getString("NAME") %>" style="width:150px;" /></td>
          </tr>
          <tr>
            <td align="right">Passwort: </td>
            <td><input name="textPASS" type="password" id="textPASS" value="<%= rsa.getString("PASS") %>"  style="width:150px;" /></td>
          </tr>
          <tr>
            <td align="right">Profil: </td>
            <td><label>
              <select name="textPROFILE" id="textPROFILE">
                <option value="user" <%=(("user".toString().equals(rsa.getString("PROFILE")))?"selected=\"selected\"":"")%>>user</option>
                <option value="admin" <%=(("admin".toString().equals(rsa.getString("PROFILE")))?"selected=\"selected\"":"")%>>admin</option>
                            </select>
            </label></td>
          </tr>
          <tr>
