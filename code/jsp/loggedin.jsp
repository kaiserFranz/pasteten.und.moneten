...
rs = statement.executeQuery("SELECT * FROM user WHERE NAME = '"+request.getParameter("txtName")+"' AND PASS = '"+request.getParameter("txtPass")+"' AND STATUS = 1"); 
...
<% } %></div>
   ...
   String profil = "false";
   if(rs.getString("PROFILE").equals("admin")) profil = "true" ;
   session.setAttribute( "theID", id );
   session.setAttribute( "theName", name );
   session.setAttribute( "isAdmin", profil );
   out.println(session.getAttribute("theID"));
      ...
      <p><% response.sendRedirect("index.jsp"); %>
      </p>
