...
connection = DriverManager.getConnection(connectionURL, "admin", "admin"); 
statement = connection.createStatement(); 
rsa = statement.executeQuery("SELECT * FROM view_aktive WHERE bilanzID = " + request.getParameter("bilanzID") + varSel + " ORDER BY strukID "); 
//
ResultSet rsp = null; 
Class.forName("com.mysql.jdbc.Driver").newInstance(); 
connection = DriverManager.getConnection(connectionURL, "admin", "admin"); 
statement = connection.createStatement(); 
rsp = statement.executeQuery("SELECT * FROM view_passive WHERE bilanzID = " + request.getParameter("bilanzID") + varSel + " ORDER BY strukID "); 
...
<% } %></div>
    ...
    <div id="content"><!-- InstanceBeginEditable name="work" --> <strong> Bilanz</strong> | <a href="income.jsp?bilanzID=<%= request.getParameter("bilanzID") %>">Erfolgsrechnung</a> | <a href="analysis.jsp?bilanzID=<%= request.getParameter("bilanzID") %>">Auswertungen</a> <br />
    &nbsp;
      <table width="100%" border="0">
      <tr>
        <td width="49%"><strong><font size="4">Aktiven</font></strong></td>
            <td width="2%">&nbsp;</td>
            <td width="49%"><strong><font size="4">Passiven</font></strong></td>
        </tr>
      <tr>
        <td valign="top"><table width="100%" cellpadding="0" cellspacing="0">
          ...
          <% teil = rsa.getString("strukTeil");
            ...
            %>
              <tr>
                <td width="1%" height="17" nowrap="nowrap" class="TD_LINE"><a href="balance.jsp?bilanzID=<%= request.getParameter("bilanzID") %>&strukID=<%= rsa.getString("strukID") %>&akt=plus" class="PLUS">+</a>&nbsp;</td>
                <td width="1%" nowrap="nowrap" class="TD_LINE"><a href="balance.jsp?bilanzID=<%= request.getParameter("bilanzID") %>&strukID=<%= rsa.getString("strukID") %>&akt=minus" class="MINUS">&#8211;</a>&nbsp;&nbsp;</td>
			...
			if(!rsp.getString("strukTeil").equals(teil)) {%>
              ...
              <tr>
                <td width="1%" height="17" nowrap="nowrap" class="TD_LINE"><a href="balance.jsp?bilanzID=<%= request.getParameter("bilanzID") %>&strukID=<%= rsp.getString("strukID") %>&akt=plus" class="PLUS">+</a>&nbsp;</td>
                <td width="1%" nowrap="nowrap" class="TD_LINE"><a href="balance.jsp?bilanzID=<%= request.getParameter("bilanzID") %>&strukID=<%= rsp.getString("strukID") %>&akt=minus" class="MINUS">&#8211;</a>&nbsp;&nbsp;</td>
